...
Default values are set in all of FACavian's functions so you can easily explore them without worrying about setting the underlying demographic parameters.  For a preliminary run parameters will be called automatically behind the scenes using a function called param_set(), which has defaults as given in Runge and Marra (2004).  
...
If you run this code you'll get a big dataframe (note that iteractions is set to a small number to make it run fast)
```{r, eval=FALSE}
